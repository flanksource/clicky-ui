...
Use property mode for a detail view whose values can be edited individually. The following complete component starts with a populated service, expands its nested connection into aligned rows, and keeps the identifier read-only.
...
Click Name, change its text, and cancel: the preview keeps the original value. Edit it again and save to commit the draft. Repeat with Connection → Host to check a nested value; Identifier has no edit button. The Storybook demo extends this example with credentials, an enum, and tags.

<figure class="component-demo">
  <iframe src="/storybook/iframe.html?id=forms-jsonschemaform-properties--editable&amp;viewMode=story" title="Editable property rows JSON Schema form demo" loading="lazy"></iframe>
  <figcaption><strong>Editable property rows</strong> · <a href="/storybook/?path=/story/forms-jsonschemaform-properties--editable">Open in Storybook</a></figcaption>
</figure>
