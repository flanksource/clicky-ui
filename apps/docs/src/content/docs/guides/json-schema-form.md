...
| `layout.mode` | Presentation | Value updates |
| --- | --- | --- |
| `stacked` | Labels above controls; supports schema-defined column grids. | Each control edit calls `onChange`. |
| `inline` | Aligned label/value columns, with width caps. | Each control edit calls `onChange`. |
| `properties` | Property rows with indented nested labels and aligned value previews. | Click a value to edit a draft; Save commits it and Cancel discards it. |
...
Menu choices override the initial size and layout props. Disable the menu when a screen must keep a fixed layout. Field-level `x-layout` and `x-label-position` hints control stacked/inline rendering; property mode keeps fields in property rows. Set `layout.help: "hover"` to move descriptions behind a help affordance, or use `x-help-display` on an individual field.
...
Fixed nested objects expand into indented rows. Arrays and maps show a count until opened for editing, where they use the existing schema controls. Saving a nested value rebuilds the root object through `onChange`. A field marked `readOnly`, or a form with `readOnly={true}`, displays values without edit buttons. Use `hideReadOnlyFields` to omit schema-declared read-only fields entirely.

Try the [property-row example](/guides/json-schema-form-examples/#property-rows) to compare Save and Cancel behavior.

## Server validation errors

Pass authoritative validation results through `errors`. Each error uses an RFC 6901 JSON Pointer into the root value; array indices are path segments, for example `/servers/0/host`.
...
<JsonSchemaForm
  ...
  onChange={setValue}
  errors={[
    { instancePath: "/name", keyword: "required", message: "Enter a name." },
    { instancePath: "", message: "Review the connection before saving." },
  ]}
/>
```

Exact field matches appear beside their controls. Root errors and paths that do not match a rendered field appear in the form summary. The host owns validation, error updates, and submission decisions; supplying errors does not block `onChange`.
