...
const NAV = [
  {
    section: undefined,
    items: [{ key: "dashboard", label: "Dashboard", icon: UiLayoutDashboard }],
  },
  {
    section: "Operations",
    items: [
...
const PLANS = Array.from(
  { length: 40 },
  (_, i) => `Scheme-G${String(36031 + i * 137).padStart(7, "0")}`,
);

export function AppShellDemo() {
  // Nav items are <a> links driven by an in-memory adapter; the active item is
  // derived from the adapter's pathname rather than local component state.
  const router = useMemoryRouter("/policies");
  return (
    <DemoSection
      id="app-shell"
      title="AppShell"
      description="Sidebar-first application shell: a full-height dark nav rail owns the brand + collapse toggle and renders grouped, icon'd nav sections as <a> links; the top bar (search, actions) sits to its right; the body has a fixed bodyHeader + bodyActions row over a bodySidebar | body-main split. Toggle by the logo to collapse the rail."
    >
      ...
      <div className="p-density-4">
        <Panel title={plan} {...(tab === "overview" ? { count: 299 } : {})}>
          <p className="text-sm text-muted-foreground">
            Nav "{active}" · tab "{tab}" · plan "{plan}" — body-main scrolls
            independently of the body sidebar.
          </p>
        </Panel>
      </div>
    </AppShell>
