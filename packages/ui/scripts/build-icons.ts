 ...
 */
// buildRegistryModule emits src/icons/registry.ts: every outline component keyed
// by its own `__consumerName`, plus a ready-made fallback provider. The map is
// built from the components' metadata rather than re-deriving names here, so it
// cannot drift from what the components actually declare.
function buildRegistryModule(
  generated: { component: string; file: string }[],
): string {
  const seen = new Set<string>();
  const outline = generated.filter(({ component }) => {
    if (
      component.endsWith("Filled") ||
      component.endsWith("Dark") ||
      seen.has(component)
    )
      return false;
    seen.add(component);
  ...
  const imports = outline
    .map(
      ({ component, file }) =>
        `import { ${component} } from "./components/${file.replace(/\.tsx?$/, "")}";`,
    )
    .join("\n");
  return `// Generated by scripts/build-icons.ts — do not edit by hand.
import { createElement, type ComponentType } from "react";
import type { FallbackIconComponent } from "../data/icon-fallback";
${imports}

type NamedIcon = ComponentType<{ size?: string | number; className?: string; title?: string }> & {
  __consumerName?: string;
};

const ALL: NamedIcon[] = [
${outline.map(({ component }) => `  ${component} as NamedIcon,`).join("\n")}
];

/** Every outline icon keyed by the consumerName in icon-selections.json. */
export const iconsByConsumerName: Record<string, NamedIcon> = Object.fromEntries(
  ALL.flatMap((icon) => (icon.__consumerName ? [[icon.__consumerName, icon]] : [])),
);

/**
 * A fallback icon provider backed by this package's own generated set, for
 * schema-driven surfaces that carry an icon as a runtime string (x-enum-icons,
 * x-icon, <Icon name="...">). Register it with setFallbackIconProvider. Pass an
 * \`next\` provider to chain a consumer's own resolver for names this set does
 * not cover (product logos, vendor marks).
 */
export function clickyIconProvider(
  next?: FallbackIconComponent,
): FallbackIconComponent {
  return function ClickyIcon({ name, className, size, alt }) {
    const icon = name ? iconsByConsumerName[name] : undefined;
    if (icon) {
...
function normalizeSvg(
  ...
  const rootStrokeLinecap = (svgOpen.match(/\bstroke-linecap="([^"]+)"/i) ||
    [])[1];
  const rootStrokeLinejoin = (svgOpen.match(/\bstroke-linejoin="([^"]+)"/i) ||
    [])[1];
  ...
  if (rootHasInherit) {
    ...
    if (!startsWithG) {
      ...
      if (rootFill) {
        const fill =
          opts.recolor && /^(#000|#000000|black)$/i.test(rootFill)
            ? "currentColor"
            : rootFill;
      ...
      if (rootStroke) {
        const stroke =
          opts.recolor && /^(#000|#000000|black)$/i.test(rootStroke)
            ? "currentColor"
            : rootStroke;
        gAttrs.push(`stroke="${stroke}"`);
      }
      if (rootStrokeWidth) gAttrs.push(`stroke-width="${rootStrokeWidth}"`);
      if (rootStrokeLinecap)
        gAttrs.push(`stroke-linecap="${rootStrokeLinecap}"`);
      if (rootStrokeLinejoin)
        gAttrs.push(`stroke-linejoin="${rootStrokeLinejoin}"`);
    ...
    const wrapperHasFillOrStroke =
      wrapperTag?.name === "g" &&
      (hasAttribute(wrapperTag.attrs, "fill") ||
        hasAttribute(wrapperTag.attrs, "stroke"));
  ...
  out = out.replace(/\bstyle="([^"]*)"/g, (_attribute, css: string) => {
    const declarations = css
      ...
      .map((declaration) => {
        ...
        if (colon < 1)
          throw new Error(`Invalid SVG style declaration "${declaration}"`);
        const property = declaration
          .slice(0, colon)
          .trim()
          .replace(/-([a-z])/g, (_, letter: string) => letter.toUpperCase());
        return `${JSON.stringify(property)}: ${JSON.stringify(declaration.slice(colon + 1).trim())}`;
      });
...
export async function buildIcons({
  ...
  function variantCount(r: SelectionRow): number {
    ...
    if (r.dark && r.dark !== "skip" && r.dark !== "maintain") n++;
    return n;
  }
  ...
  for (const r of sel.rows) {
    ...
    const base = componentNameForSelection(r);
    if (SUB_ICON_OVERRIDES.has(base)) {
      if (r.group === "programming")
        throw new Error(
          `Programming icon "${r.consumerName}" conflicts with ${base}`,
        );
      continue;
    }
    ...
    if (r.group === "programming" || currentRow.group === "programming")
      throw new Error(
        `Programming icon "${r.consumerName}" conflicts with "${current}" at ${base}`,
      );
  ...
  for (const row of sel.rows) {
    ...
    const baseName = componentNameForSelection(row);
    ...
    if (row.dark && row.dark !== "skip" && row.dark !== "maintain") {
      variants.push({ slot: "dark", spec: row.dark, suffix: "Dark" });
    }
    ...
    if (aliasTarget && rowByName.has(aliasTarget)) {
      ...
      const reExportOutline =
        aliasHasOutline &&
        targetHasOutline &&
        targetRow.outline === row.outline;
      // Filled export only if both target and alias chose the same filled.
      const reExportFilled =
        aliasHasFilled && targetHasFilled && targetRow.filled === row.filled;
      ...
      const aliasFullyCovered =
        (!aliasHasOutline || reExportOutline) &&
      ...
      if (aliasFullyCovered && (reExportOutline || reExportFilled)) {
        ...
        if (aliasBase !== targetBase) {
          ...
          if (reExportFilled)
            exports.push(`${targetBase}Filled as ${aliasBase}Filled`);
          ...
          if (reExportFilled)
            generated.push({ component: `${aliasBase}Filled`, file });
        }
        continue;
      }
    }

    // Fetch + emit per variant. Source and dark artwork share one component module.
    const parts: string[] = [
    ...
    if (subRecipes.length > 0) {
      ...
      if (variantPayload.outline)
        subBases.push({ base: variantPayload.outline, suffix: "" });
      if (variantPayload.filled)
        subBases.push({ base: variantPayload.filled, suffix: "Filled" });
      ...
      for (const recipe of subRecipes) {
        for (const { base, suffix: variantSuffix } of subBases) {
          ...
          const tintBase =
            baseLookup === "Shield" && variantSuffix === "Filled";
          const composedInner = composeWithSubIcon(
            base.inner,
            base.viewBox,
            recipe,
            { tintBase },
...
`,
  ...
  // are pure aliases.
  {
    ...
    // Import each icon straight from its component module, not the ./index
    // barrel: the barrel re-exports this file, so importing it back would form
    // an import cycle (index → changeIconAliases → index), which defeats
    // Vite's granular HMR.
    const moduleByComponent = new Map(
      generated.map((g) => [g.component, g.file.replace(/\.tsx?$/, "")]),
    );
    const namesByModule = new Map<string, string[]>();
    for (const name of [...importNames].sort()) {
      const mod = moduleByComponent.get(name)!;
      namesByModule.set(mod, [...(namesByModule.get(mod) ?? []), name]);
    }
    const importLines = [...namesByModule.entries()]
      .map(
        ([mod, names]) =>
          `import { ${names.join(", ")} } from "./components/${mod}";`,
      )
      .join("\n");
...
`;
  ...
  // Barrel index.
  const programmingRows = sel.rows.filter((row) => row.group === "programming");
  const programmingImports: string[] = [];
  const programmingEntries: string[] = [];
  for (const row of programmingRows) {
    if (
      ...
      throw new Error(
        `Programming icon "${row.consumerName}" is missing catalog metadata or a theme variant`,
      );
    }
    const component = componentNameForSelection(row);
    programmingImports.push(
      `import { ${component}, ${component}Dark } from "./components/${component}";`,
    );
    programmingEntries.push(
      `  { id: ${JSON.stringify(row.consumerName)}, componentName: ${JSON.stringify(component)}, label: ${JSON.stringify(row.label)}, family: ${JSON.stringify(row.family)}, concept: ${JSON.stringify(row.concept)}, variation: ${JSON.stringify(row.variation)}, tone: ${JSON.stringify(row.tone)}, source: ${JSON.stringify(row.outline)}, headerVerified: ${row.outline.startsWith("jb-download:")}, icon: ${component}, darkIcon: ${component}Dark },`,
    );
  }
  await writeFile(
    join(stagingRoot, "programmingCatalog.ts"),
    `// Generated by scripts/build-icons.ts — do not edit by hand.
import type { IconComponent } from "./types";
${programmingImports.join("\n")}

export type ProgrammingIconEntry = {
  id: string;
...
export const programmingIconCatalog: ProgrammingIconEntry[] = [
${programmingEntries.join("\n")}
];
`,
  );
  ...
  for (const { component, file } of generated) {
    if (seenBarrel.has(component)) continue;
    seenBarrel.add(component);
    const moduleName = file.replace(/\.tsx?$/, "");
    lines.push(`export { ${component} } from "./components/${moduleName}";`);
  }
  lines.push(`export type { IconProps, IconComponent } from "./types";`);
  lines.push(
    `export { changeIconAliases, getChangeIcon } from "./changeIconAliases";`,
  );
  ...
  // Runtime-name registry. Schema-driven surfaces (x-enum-icons, x-icon,
  // <Icon name="...">) carry an icon as a STRING, which no import can resolve.
  // This maps each consumerName back to its outline component so a consumer can
  // register one provider instead of hand-rolling the __consumerName lookup.
  // It lives in the generated tree — and behind its own module — so an app that
  // never registers it does not pull every icon into its bundle.
  await writeFile(
    join(stagingRoot, "registry.ts"),
    buildRegistryModule(generated),
  );
  ...
  const noticeLines: string[] = [
    "# NOTICE — @flanksource/clicky-ui/icons",
    "",
    ...
    "- JetBrains IntelliJ Platform icons with embedded Apache headers — Apache 2.0 — https://github.com/JetBrains/intellij-community",
    "- JetBrains catalog ZIPs marked jb-download-unverified have no embedded license header; their license is not verified by this import.",
  ...
  for (const row of sel.rows) {
    if (row.outline && row.outline !== "skip" && row.outline !== "maintain") {
      noticeLines.push(
        `| ${componentNameForSelection(row)} | ${row.outline} |`,
      );
    }
    if (row.filled && row.filled !== "skip" && row.filled !== "maintain") {
      noticeLines.push(
        `| ${componentNameForSelection(row)}Filled | ${row.filled} |`,
      );
    }
    if (row.dark && row.dark !== "skip" && row.dark !== "maintain") {
      noticeLines.push(
  ...
  console.log(
    `Generated ${seenBarrel.size} components across ${new Set(generated.map((g) => g.file)).size} files.`,
  );
