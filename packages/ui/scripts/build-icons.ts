...
function buildRegistryModule(generated: { component: string; file: string }[]): string {
  const seen = new Set<string>();
  const outline = generated.filter(({ component }) => {
    if (component.endsWith("Filled") || seen.has(component)) return false;
    seen.add(component);
    return true;
  });
  const imports = outline
    .map(({ component, file }) => `import { ${component} } from "./components/${file.replace(/\.tsx?$/, "")}";`)
    .join("\n");
...
`,
  ...
  // are pure aliases.
  {
    ...
    const importLines = [...namesByModule.entries()]
      .map(([mod, names]) => `import { ${names.join(", ")} } from "./components/${mod}";`)
      .join("\n");
...
`;
  ...
  const lines: string[] = ["// Generated by scripts/build-icons.ts — do not edit by hand."];
  ...
  lines.push(`export { clickyIconProvider, iconsByConsumerName } from "./registry";`);
  ...
  await writeFile(join(stagingRoot, "registry.ts"), buildRegistryModule(generated));
