...
The tokenizers come from cel-go's ANTLR grammar, `text/template`'s lexer and gomplate's live function registries, none of which is readable outside a Go toolchain. `scripts/vendor-expressions.ts` clones gomplate, runs its generator, and commits the result:
...
pnpm vendor:expressions:check                    # fail if stale or hand-edited
```

The whole language source tree is copied, not just the generated JSON: the completion, hover and path-expression runtime changes with the generated shape, and a second copy would drift within a release. `src/expressions/VENDOR` records the gomplate commit it came from.

A weekly workflow re-runs the vendoring and opens a PR when it changes; a PR touching `src/expressions/lang` runs `vendor:expressions:check` against a fresh clone.

**Fix language behaviour in gomplate, not here.** An edit under `src/expressions/lang` is overwritten by the next vendoring, and `vendor:expressions:check` fails the PR that makes one.
