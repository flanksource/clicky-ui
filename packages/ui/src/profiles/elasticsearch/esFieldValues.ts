...
/**
 * makeFieldValueLookup binds a lookup to a connection and an index. The scope
 * is compiled server-side against `params`, so a sibling condition that
 * interpolates `{{.params.…}}` narrows the suggestions the same way a run would.
 */
export function makeFieldValueLookup(options: {
  ...
  return ({ field, search }) => ({
    ...
    fetch: async (query) => {
      ...
      return postValues(baseUrl, body);
    },
  });
}
