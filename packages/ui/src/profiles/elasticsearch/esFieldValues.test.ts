...
describe("makeFieldValueLookup", () => {
  ...
  it("surfaces a scope that cannot compile without widening the lookup", async () => {
    const fetchMock = vi.fn().mockResolvedValue(respondWith("condition has no value", 422));
    vi.stubGlobal("fetch", fetchMock);
