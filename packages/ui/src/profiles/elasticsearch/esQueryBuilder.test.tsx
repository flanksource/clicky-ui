...
vi.mock("./esQueryCompile", async (importOriginal) => {
  const original = await importOriginal<typeof import("./esQueryCompile")>();
  return {
    ...original,
...
const fields: EsFieldMapping[] = [
  ...
  { name: "observed_at", dataType: "long", aggregatable: true },
...
describe("EsQueryBuilder", () => {
  ...
  it("renders the selected mapped time field", () => {
  ...
  it("requires an epoch unit for a numeric time field", () => {
    const html = render({ timeField: "observed_at" });
    expect(html).toContain('aria-label="Time field format"');
    expect(html).toContain('aria-required="true"');
  });

  it("uses native date values without an epoch selector", () => {
    const html = render({ timeField: "@timestamp" });
    expect(html).not.toContain('aria-label="Time field format"');
  });
...
describe("EsQueryBuilderField compilation", () => {
  ...
  it("sends the declared parameter defaults and roles to /compile", () => {
    ...
    expect(compileInputs[0]?.index).toBe("jaeger-span*");
    const body = JSON.parse(compileRequestBody(compileInputs[0]!));
    expect(body.index).toBe("jaeger-span*");
...
describe("time field mappings", () => {
  const mapped: EsFieldMapping[] = [
    { name: "created_at", types: ["date"] },
    { name: "startTimeMillis", types: ["long"] },
    { name: "sequence", types: ["integer"] },
    { name: "duration", types: ["double"] },
    { name: "service.name", types: ["keyword"] },
  ];

  it("offers native dates and integral fields", () => {
    expect(timeFieldMappings(mapped).map((field) => field.name)).toEqual([
      "created_at",
      "startTimeMillis",
      "sequence",
    ]);
  ...
  it("requires an epoch unit for numeric and custom fields", () => {
    expect(timeFieldFormatRequired(mapped, "created_at")).toBe(false);
    expect(timeFieldFormatRequired(mapped, "startTimeMillis")).toBe(true);
    expect(timeFieldFormatRequired(mapped, "custom_timestamp")).toBe(true);
    expect(timeFieldFormatRequired(mapped, "")).toBe(false);
  });

  it("clears a numeric epoch unit when a native date field is selected", () => {
    expect(
      selectTimeField(
        { timeField: "startTimeMillis", timeFieldFormat: "epoch_millis" },
        mapped,
        "created_at",
      ),
    ).toEqual({ timeField: "created_at" });
  });
});
