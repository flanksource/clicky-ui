...
export const profileEditorSections = [
  { id: "general", label: "General", hint: "Name, namespace, render mode" },
  {
    id: "source",
    label: "Source & Query",
    hint: "Provider, connection, sample",
  },
  ...
  { id: "parameters", label: "Parameters", hint: "Named query inputs" },
  {
    id: "processors",
    label: "Processors",
    hint: "Ordered post-query transformations",
  },
...
export const profileGeneralOptionKeys = [
  "imports",
  "aliases",
  "output",
  "ignore",
];
export const profileProcessorKeys = ["processors"];
...
export function profileEditorSectionStatus({
  ...
  return {
    ...
    processors: { badge: processors ? String(processors) : undefined },
    raw: {},
  };
}
...
export function profileSchemaProjection(keys: string[]): JsonSchemaObject {
  ...
  return {
    type: "object",
    properties,
    required: (profileSchema().required ?? []).filter((key) =>
      keys.includes(key),
    ),
  };
}
...
export function providerConnectionSchema(): JsonSchemaObject {
  const connection =
    profileSchema().properties?.provider?.properties?.connection;
  if (!connection) {
    throw new Error("profile schema is missing provider.connection");
  }
  return { type: "object", properties: { connection } };
}
