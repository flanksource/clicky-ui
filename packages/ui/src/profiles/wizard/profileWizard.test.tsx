...
import { ProfileFieldManager } from "../fields/profileFieldManager";
import { profileFormatOptions, profileUnitOptions } from "../fields/profileFieldIcons";
import {
...
describe("large profile field sets", () => {
  ...
  it("uses canonical Format and Unit comboboxes with explanatory help", () => {
    const html = renderToStaticMarkup(
      <ProfileFieldManager
        discovered={discoveredFields}
    ...
    // The options live in the combobox menu, which opens on demand; what the
    // closed form shows is the named control and the help behind its `?`.
    expect(html).toContain('aria-label="Format"');
    expect(html).toContain('aria-label="About Format"');
    expect(html).toContain('aria-label="Unit"');
    expect(html).toContain("Max width (characters)");
    expect(profileFormatOptions.map((option) => option.value)).toEqual(
      PROFILE_COLUMN_FORMAT_OPTIONS.map((option) => option.value),
    );
    expect(profileUnitOptions.map((option) => option.value)).toEqual(
      PROFILE_COLUMN_UNIT_OPTIONS.map((option) => option.value),
    );
  });
});
